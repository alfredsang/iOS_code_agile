...
#define TEXTFIELDLEFTMARGIN 22
#define TEXTFIELDTOPMARGIN 40 +40
#define TEXTFIELDHEIGHT 68
...
-(void)initTextView{
	...
	CGRect descriptionRect = CGRectMake(26, TEXTFIELDTOPMARGIN+65, 320-26*2, TEXTFIELDHEIGHT);
    
   
    m_labelView = [[UITextField alloc] init];
    m_labelView.borderStyle  = UITextBorderStyleNone;

//    m_labelView.layer.borderColor = [[UIColor darkGrayColor] CGColor];
//    m_labelView.layer.borderWidth = 1;
//    m_labelView.placeholder = @"请输入您的email地址"; 
    m_labelView.frame = CGRectMake(32, 45+32, 320-28*2, 30);
...
//    m_textView.layer.backgroundColor = [[UIColor darkGrayColor] CGColor];
...
//    LOG_EXPR(_l_buttonView);
    ...
    if ([[NSUserDefaults standardUserDefaults] objectForKey:@"feedback_email_addr"]) {
        m_labelView.text = [NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults] objectForKey:@"feedback_email_addr"]];
    }
...
- (IBAction)tapOnCommentBtn:(id)sender 
{
    if (!isReachability)
    {
        [[UIApplication sharedApplication] showNetworkConnectionFailedAlertViewWithMessage:@"无法查看及\n发表评论！"];
        return;
    }
    ...
    if (emailAddr == nil|| [emailAddr isEqualToString:@""]) {
        ...
        [[NSUserDefaults standardUserDefaults] setObject:emailAddr forKey:@"feedback_email_addr"];
    }
...
//    
    ...
    NSString *url =  [[NSString stringWithFormat:@"%@%@",@"127.0.0.1", @"?m=api_user&a=guestbookadd&"] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
